...
var initTasksModel = mongoose.model('initTasks', initTasks);


var initUsers = new Schema({
    id:Number,
    username:String,
    password:String,
    email:String,
...
async function init() {
    ...
    await initUsersModel.find({}, function(e,d) {
        d.forEach(item => {
            initUsersModel.findOneAndDelete({__v:0}, function(ee, dd) {
            })
        })
    })

    await initTasksModel.find({}, function(e,d) {
        d.forEach(item => {
            initTasksModel.deleteOne({__v:'0'}, function(ee, dd) {
    ...
    tempUser.forEach(item => {
        const newInitUsersModel = new initUsersModel(item)
        newInitUsersModel.save(function(e,d){

        })
    })

    task.forEach(item => {
        const newInitTasksModel = new initTasksModel(item)
        newInitTasksModel.save(function(e,d){
...
async function configure() {
    ...
    await initTasksModel.find({}, function(e,d) {
        d.forEach(item => {
            initTasksModel.findOneAndUpdate({_id:item._id}, {createdDate:getTime()} , {new:true, upsert:true}, function(e,d) {
            })
        })
    })

    initUsersModel.find({}, function (e, d) {
        d.forEach(async item => {
            const hash = await bcrypt.hash(item.password, 10)
            initUsersModel.findOneAndUpdate({id:item.id}, {password:hash}, {new:true, upsert:true}, function(e,d) {

            })
        })
        initTasksModel.find({}, function (ee, dd) {
            for (let i = 0; i < 200; i++) {
                const nnn = d.find(item => dd[i].userId == item.id)
                if (nnn) {
                    initTasksModel.findOneAndUpdate({id:i+1}, {ref:nnn._id}, {new:true, upsert:true}, function (e, d) {
